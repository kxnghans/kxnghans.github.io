...
*   **Styling**: Tailwind CSS 3, integrated with custom Neumorphic tokens in `index.css`.
*   **Build/Routing**: Vite 7. The app is an SPA using custom state-based routing (`activePage` in `App.jsx`), NOT `react-router-dom`.
*   **Testing**: Vitest with React Testing Library (RTL).
*   **Form/Validation**: React Hook Form, Sonner for Toast notifications.

---

## 🛑 Operational Constraints
*   **Zero-Tolerance ESLint Policy**: All generated code must be free of linting errors (`npm run lint`). Do not disable rules (e.g., `// eslint-disable-next-line`) without explicit user permission.

...
- **High-Performance Search Engine (`src/utils/searchEngine.ts`)**: Zero-dependency, in-memory search scoring engine executing multi-token matching, field-weighted scoring hierarchy (Exact > Title > Tags > Subtitle > Summary > Content), tech term alias expansions (`c++`/`cpp`, `next.js`/`nextjs`, `ai`/`ml`), Levenshtein typo tolerance, and category-diverse smart recommendations (`getRecommendations`) for empty search states. Queries run through React's `useDeferredValue` in `SearchContext.tsx` with LRU caching to eliminate UI hitching.
- **Media Asset Strategy (Google Cloud Storage CDN)**: Production images (49 items across projects, skills, work, education, honors, certifications, community, profile, and brand) are hosted on Google Cloud Storage under `gs://portfolio_showcase/images/` and served via edge CDN (`https://storage.googleapis.com/portfolio_showcase/images`). Assets are registered centrally in `src/data/assets.ts` (`ASSET_URLS`, `getAssetUrl`). UI consumers load assets asynchronously via `LazyImage.tsx` with skeleton shimmer states, native `loading="lazy"`, and `decoding="async"`.
- **AST ESLint Image Guardrail**: An ESLint AST rule (`no-restricted-syntax`) strictly forbids hardcoding `.webp`, `.png`, `.jpg`, and `.jpeg` path literals anywhere in the codebase outside `src/data/assets.ts`, preventing accidental bypassing of the CDN asset registry.
- **Workbox Runtime Caching**: Production service worker precaches application shell assets (~528 KiB) while runtime GCS image requests are cached via a `CacheFirst` strategy with `cacheableResponse: { statuses: [0, 200] }` to safely cache cross-origin CDN responses.

...
| :------- | :---------------------------- | :-------------------------------------------------------------- | :----------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------- |
| **Low**  | End-to-End Test Automation    | Comprehensive browser-level user flow testing across viewports. | 15 Vitest suites (69 unit/integration tests) passing in JSDOM environment.     | Implement Playwright E2E test suite to automate browser-level validation of search, routing, and modal interaction journeys.     |
| **Low**  | Cloud Bucket Fallback Staging | High-resolution media hosted locally vs GCP Cloud Storage.      | Stored locally as optimized WebP in `public/assets/generated/` (6.0MB total).  | Maintain local assets as default SSOT; stage GCP Cloud Storage bucket for optional CDN asset streaming if repository size grows. |
